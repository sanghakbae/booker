...
/**
 * Stamp this build so a client running an older copy can notice.
 *
 * GitHub Pages serves HTML with max-age=600 and in-app browsers hold it far
 * longer, so a fix can be live while a tester still runs the previous bundle.
 * The id goes into a file the app fetches with no-store, and into a module the
 * app compiles in; a mismatch means the page is stale.
 */
const buildId = `${new Date().toISOString().replace(/[-:.TZ]/g, "").slice(0, 14)}`;
await writeFile("public/build-id.json", JSON.stringify({ id: buildId }));
await writeFile(
  "src/lib/build-id.ts",
